...
if($op=="edit"){
	...
	$rsdata_count= $cndb->recordCount($rsdata);
		...
		$sq_item_dates = "SELECT date, end_date FROM `dhub_dt_content_dates` WHERE (`id_content` = ".quote_smart($id).") order by date ASC; ";
...
<td><!--{0}.-->&nbsp;</td>
<td><input type="text" name="date_add[{0}]" id="date_add_{0}" value=""  class="small_width date-pick required" ></td>
...
<td><a href="#" onclick="del_date_row({0});return false;">del</a></td>
</tr>
...
<td><!--{0}.-->&nbsp;</td>
<td><input type="text" name="date_add['.$row.']" id="date_add_'.$row.'" value="'.$rowval.'"  class="small_width date-pick required" ></td>
...
<form class="admformX rwdform rwdfull" id="cont_basic" name="rage" method="post" action="adm_posts.php"  enctype="multipart/form-data">
...
<?php echo $pgtitle; ?>
  <table  border="0" cellspacing="1" cellpadding="5" align="center" class="tims" style="width:100%" >
	...
	<tr>
      <td nowrap="nowrap" style="width:150px"><label for="id_parent">Parent Menu:</label></td>
      <td colspan="3">
    ...
    <tr>
    	...
    	<td colspan="3">
		<input type="text" name="title" id="article_title" value="<?php echo $title; ?>" class="text_full required"></td>
    </tr>
...
/*  -----------------------------------------------------------  
?>	
<tr><td nowrap="nowrap"><label>Sector / Project Link:</label></td> <td colspan="3">
<table style="margin:0 "> <tr> <td style="width:40%"> <select id="sector_id" name="sector_id" onchange="javascript: show_projects(); "> <?php echo $ddSelect->dropper_select("dhub_app_sector", "sector_id", "title", $sector_id, "Select Sector") ?> </select> </td> <td> <span id="box_project"><input name="project_id" id="project_id" type="text" readonly placeholder="Select Project" ></span></td> </tr> </table>
</td></tr>
<?php 
/*	-----------------------------------------------------------
...
?>	
	...
	<?php /*?><tr>
    	<td nowrap="nowrap"><label for="id_section">Display Type</label></td>
    	<td colspan="3"><select name="id_section" id="id_section" class="required">
    ...
    <tr>
		...
		<table style="margin:0;" id="event_dates_table" border="1" class="full">
<thead>
<tr>
<th>&nbsp;</th>
...
<hr />
    ...
    <tr>
    	...
    	<td colspan="3"><?php include("fck_rage/article_sm.php"); ?></td>
    </tr>
    ...
    <tr>
      ...
      <td colspan="3">
	  <input type="text" name="ev_location" id="ev_location" value="<?php echo $ev_location; ?>" class="required text_full" />
	  </td>
	</tr>	
...
<input type="checkbox" id="booking_form" name="booking_form" <?php echo yesNoChecked($book_form); ?>  class="radio"/>	<strong>Show Registration Form </strong>
	...
	<tr>
      <td nowrap="nowrap"></td>
      <td>
	  <label class="inline" style="color:#900">
<input type="checkbox" name="published" id="published" <?php echo $published; ?> class="radio"/> <strong>Is Published</strong>
</label>
	  </td>
	  <td colspan="2">
	  <label class="inline" style="color:#900">
<input type="checkbox" name="approved" id="approved" <?php echo $approved; ?> class="radio"/> <strong>Is Approved</strong>
</label>
	  </td>
	</tr>
...
<?php /*?><link rel="stylesheet" href="scripts/jquery.datepick.css" id="theme">
<script src="scripts/jquery.datepick.js"></script><?php */?>
