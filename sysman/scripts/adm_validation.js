...
function valid_menu() 
{
	...
	if(clip(document.rage.id_section.value).length<1){
		   alert("Menu Section is required.");
			document.rage.id_section.focus();
		  return false;
	}
	
	if(clip(document.rage.id_type_menu.value).length<1){
	...
	else
	{
		if(clip(document.rage.id_type_menu.value)==2){
			...
			if(clip(document.rage.id_parent.value).length<1){
