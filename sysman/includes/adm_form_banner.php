...
if($op=="edit"){
	...
	$sqdata="SELECT `id`, `id_gallery_cat`, `filename`, `description`, `seq`, `published`, `title`, `title_sub`, `id_link` FROM `dhub_dt_gallery_photos` WHERE  (`id` = ".quote_smart($id).")";
	...
	$rsdata_count= $cndb->recordCount($rsdata);
		...
		$pgtitle				="<h2>Edit Banner Item</h2>";
		...
		$formname			= "banner_edit";
		}
	}
} elseif($op=="new")
	{
	
		$pgtitle				="<h2>New Banner Item</h2>";
	  ...
	  <table width="700px" border="0" cellspacing="1" cellpadding="2" align="center">
        <tr>
		 ...
		 <tr style="background:#F1F1F1;">
          <td></td>
          <td>Gallery</td>
          <td><select name="id_gallery" id="id_gallery" >
           <?php echo $ddSelect->dropper_select("dhub_dt_gallery_category", "id", "title", $id_gallery) ?>
		   </select></td>
        </tr>
		<tr>
			<td></td>
			<td><strong>Link To:</strong></td>
          	<td><select name="id_link"  id="id_link">
            	<?php echo $dispData->build_MenuSelect($dispData->menuMain, $id_link); ?>
         		 </select>
			</td>
		</tr>
         <tr style="background:#F1F1F1;">
        <td></td>
        <td> Image Title </td>
        <td><input type="text" name="title" id="title" value="<?php echo $title; ?>" maxlength="45"/></td>
      </tr>
	  <?php /*?><tr style="background:#F1F1F1;">
        <td></td>
        <td> Image Sub Title </td>
        <td><input type="text" name="title_sub" id="title_sub" value="<?php echo $title_sub; ?>" maxlength="45"/></td>
      </tr><?php */?>
        <tr style="background:#dfdfdf;">
          <td class="required">*</td>
          <td>Select Image: </td>
          <td><input type="file" name="MyFile" id="MyFile" size="45" <?php echo $image; ?> />
			...
			Yes&nbsp;&nbsp;
			<input name="change_image" type="radio" value="No" <?php echo $upload_picn; ?> id="checkbox"  class="radio"/>No				</td>
          </tr>
        ...
        <tr style="background:#dfdfdf;">
          <td>&nbsp;</td>
          <td>Image Caption </td>
          <td><textarea name="description" id="description" style="width:90%; height: 70px;"><?php echo $description; ?></textarea><br />
