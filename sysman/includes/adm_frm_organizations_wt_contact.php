...
{	
	...
	if($post['account_id'] <> ''){	
		...
		$sq_post[] = "update `dhub_reg_account` set `group_id` = ".q_si($ac_group_id).", `namefirst`=".q_si($ac_namefirst).",`namelast`=".q_si($ac_namelast).",`email`=".q_si($ac_email).",`phone`=".q_si($ac_phone).", `uservalid`=".q_si($ac_uservalid).",`published`=".q_si($ac_published)." ".$sq_pass." where `account_id` = ".q_si($post['account_id']).";  "; */
	}
	...
	if($new_contact == 'XX1')
	{
		...
		$sq_upd[] = "update `dhub_reg_account` set `published` = ".q_si($ac_published)." WHERE `account_id` = ".q_si($acc_id)."; ";
		$sq_upd[] = "update `dhub_conf_organizations` set `contact_id` = ".q_si($acc_id)." WHERE `organization_id` = ".q_si($org_id)."; ";
