...
if($op=="edit"){
	...
	if($id)
	{
		...
		$contact_id		= $fData['contact_id'];
			...
			$sq_contact	= "SELECT `account_id`, concat_ws(' ',`dhub_reg_account`.`namefirst`, `dhub_reg_account`.`namelast`) as `name` FROM `dhub_reg_account` WHERE  (`organization_id`  = ".quote_smart($id).")";	
