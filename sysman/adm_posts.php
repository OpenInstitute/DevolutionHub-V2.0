...
echo '<div style="text-align:center; padding-top:50px;"><img src="../image/icons/ajax_loader.gif" /></div>'."\r\n \r\n";
...
$adTabProps['pillars'] = array('tbn' => 'dhub_app_pillar', 'tbk' => 'pillar_id');
$adTabProps['sectors'] = array('tbn' => 'dhub_app_sector', 'tbk' => 'sector_id');
$adTabProps['locations'] = array('tbn' => 'dhub_app_location', 'tbk' => 'location_id');
$adTabProps['ministries'] = array('tbn' => 'dhub_app_ministry', 'tbk' => 'ministry_id');
$adTabProps['projects'] = array('tbn' => 'dhub_app_project', 'tbk' => 'project_id');
...
{	
	...
	*/
	...
	if($formaction == "_new")
	{
		$sq_check = "SELECT `organization` FROM `dhub_conf_organizations` WHERE (`organization` = ".q_si($org_d['organization']).")";
		...
		$sq_post = "insert into `dhub_conf_organizations`  (`organization`, `organization_seo`, `organization_website`, `organization_email`, `organization_phone`, `organization_profile`, `is_partner`, `published`) values (".q_si($org_d['organization']).", ".q_si($org_seo).", ".q_si($org_d['organization_website']).",  ".q_si($org_d['organization_email']).",  ".q_si($org_d['organization_phone']).",  ".q_si($org_d['organization_profile']).",  ".q_si($is_partner).",  ".q_si($published).")";
	}
	elseif($formaction == "_edit")
	{
		$org_id		= $post['organization_id'];
		$sq_post 	= "update `dhub_conf_organizations` set `organization`=".q_si($org_d['organization']).", `organization_seo`=".q_si($org_seo).", `organization_website`=".q_si($org_d['organization_website']).", `organization_email`=".q_si($org_d['organization_email']).", `organization_phone`=".q_si($org_d['organization_phone']).", `organization_profile`=".q_si($org_d['organization_profile']).", `published` = ".q_si($published).", `is_partner` = ".q_si($is_partner)."  where `organization_id` = ".q_si($org_id).";  "; 
	}
	//echobr($sq_post); exit;
	...
	if(isset($_FILES['uplogo']) and strlen($_FILES['uplogo']['name']) > 0) 
	{
		...
		{
			$sq_file  = "UPDATE `dhub_conf_organizations` set `logo` = ".q_si($file_name_new)."  where `organization_id` = ".q_si($org_id)."; "; 
...
{	
	...
	$ac_published	= yesNoPost(@$post['user']['published']); 
		...
		{			
			...
			$sq_upd  = "update `dhub_reg_account` set `published` = ".q_si($ac_published)." WHERE `account_id` = ".q_si($account_id)."; ";
			$cndb->dbQuery($sq_upd);
		...
		elseif($formaction == "_edit")
		{
			...
			$sq_post 		= "update `dhub_reg_account` set `group_id` = ".q_si($ac_group_id).", `organization_id` = ".q_si($ac_org_id).", `namefirst`=".q_si($ac_namefirst).",`namelast`=".q_si($ac_namelast).",`email`=".q_si($ac_email).",`phone`=".q_si($ac_phone).", `uservalid`=".q_si($ac_uservalid).",`published`=".q_si($ac_published)." ".$sq_pass." where `account_id` = ".q_si($post['account_id']).";  "; 
			//echobr($sq_post); exit;
	...
	if(isset($_FILES['upavatar']) and is_array($_FILES['upavatar']['name'])) 
	{
		...
		if($doc_result['result'] == 1) {
			$sq_files  = "UPDATE `dhub_reg_account` SET `avatar`= ".q_si($doc_result['name'])." WHERE `account_id` = ".q_si($account_id)."; ";
...
if ($formname=="conf_account" or $formname=="account_edit" or $formname=="account_new") 
{
	...
	if ($formact == "_edit" )
	{
		...
		$sqpost = "UPDATE `dhub_reg_account` set  ".implode($myCols, ', ')." where (`account_id` = ".quote_smart($account_user_id)." )" ;
		//echo $sqpost; exit;
		//$type = new posts;
	...
	if(is_array($postTitle)) 
	{
		...
		foreach($postTitle as $aKey => $aVal) 
		{
            if($aVal <> '') 
			{
				...
				$sqpost_menus = "insert IGNORE into `dhub_dt_content_parent` (`id_content`,`id_parent`) values "
				...
				if($postImage[$aKey] <> '') 
				{
					...
					$sqpost_image  = "insert ignore into `dhub_dt_gallery_photos`  "
					...
					$sqpost_pics[] = " insert IGNORE into `dhub_dt_gallery_photos_parent` ( `id_photo`,`id_content`,`rec_stamp` ) values (".quote_smart($id_image).", ".quote_smart($id_content).", ".$timePublish." );  ";			
...
if ($formname=="fm_directory_products") 
{
	...
	{	
		...
		{  
			$seq_content[]  = " insert IGNORE into `dhub_reg_equipment_supplier` ( `id_equipment`, `id_directory` ) values "
...
if ($formname=="fm_directory_cat" or $formname=="fm_directory" or $formname=="fm_equipment" ) 
{
	...
	$formprimary = " `id` ";
	if($formname == "fm_directory_cat") { $formtable = " `dhub_reg_directory_category` "; }
	if($formname == "fm_directory")     { $formtable = " `dhub_reg_directory` "; }
	if($formname == "fm_equipment")     { $formtable = " `dhub_reg_equipment` "; $formprimary = " `id_equipment` "; }
	...
	****************************************************************/
		...
		{	
			$sq_par_clean = " delete from `dhub_reg_equipment_supplier` where `id_equipment` = '".$post_id."' ";
			...
			if(count($content_parent)>0)
			{
				...
				{  
					$seq_update_content[]  = " insert IGNORE into `dhub_reg_equipment_supplier` ( `id_equipment`, `id_directory` ) values "
					." ('".$post_id."', '".$supplier."')  ";
				} 
	...
	****************************************************************/
	$status_image_update = 0;
		...
		if($post['eq_image'] <> '')
		{
			$sq_upload = "insert IGNORE  into `dhub_dt_gallery_photos`  "
			."( `title`, `filename`,  `id_gallery_cat`, `date_posted`)   values "
			...
			$sq_upload_parent = " insert IGNORE into `dhub_dt_gallery_photos_parent` " 
		...
		if(trim($_FILES['upload_name']['name']) <> '') 
		{
			...
			if($the_image['result'] == 1)
			{
				...
				$sq_update_parent = " update `dhub_reg_equipment` set `eq_image` =  '".$the_image['name']."' where " 
...
****************************************************************/
	...
	{	
		...
		$sqdatesclean = "delete from dhub_dt_content_dates where id_content = '".$id_content."'; ";
		...
		{  
			...
			{ 
			  ...
			  $seq_cont_dates[]  = " insert IGNORE into dhub_dt_content_dates (id_content, date, end_date) values ('".$id_content."', ".$dateStart.", ".$dateEnd."); ";
			} 
...
if($formname=="fm_usercat_edit" or $formname=="fm_usercat_new")
{
	...
	$sqpost="UPDATE `dhub_reg_cats` SET "
...
if($_POST['formname'] == 'forum_edit_posts')
{
		...
		$sq_post = "UPDATE `dhub_forum_posts` SET `post_content` = '" . sanitizor($_POST['post_content']) . "', `post_published` = '".$published."'  WHERE `post_id` = '" . sanitizor($_POST['post_id']) . "' "; 
		 //echo $sq_post; exit;
		$rs_post = $cndb->dbQuery($sq_post);
...
if($formname=="adm_bulk_sms") 
{
	...
	{ 
	  ...
	  foreach($results as $result) {
		...
		$sq_sms_log[] ="INSERT INTO `dhub_log_sms` 
...
if ($formname=="edit_file_single") 
{
	...
	//$type = new posts;
		...
		foreach ($post['image'] as $key => $value) 
		{
			...
			$seq_pic_update[] = "update `dhub_dt_gallery_photos` set "
	...
	$seq_pic_clean = " delete from `dhub_dt_gallery_photos_parent` where `id_photo` = ".quote_smart($id_photo)." ";
	...
	{	
		...
		{  
	$seq_pic_update[]  = " insert IGNORE into `dhub_dt_gallery_photos_parent` ( `id_photo`, `id_link`, `rec_stamp` ) values  (".quote_smart($id_photo).", '".$parent_menu[$i]."', '".$record_stamp."'); ";
		} 
	}
	...
	{	
		...
		{  
	$seq_pic_update[]  = " insert IGNORE into `dhub_dt_gallery_photos_parent` ( `id_photo`, ".$parent_title.", `rec_stamp` ) values  (".quote_smart($id_photo).", ".quote_smart($cval).", '".$record_stamp."'); ";
		} 
	}
...
if($formname == 'poll_edit' or $formname == 'poll_new') 
{
	...
	if($cur>0) $cndb->dbQuery("UPDATE `dhub_poll_questions` SET `show`=0");
	if(mysql_errno>0) $err .= mysql_errno().": ".mysql_error()."<br />";
	
	
	if($formname == 'poll_new'){
		$query = "INSERT INTO `dhub_poll_questions` (`subject`,`question`,`date`,`show`,`published`) VALUES ('".$subject."','".$question."',NOW(),'".$cur."','".$pub."')";
		//echo $query; exit;
		$cndb->dbQuery($query);
		...
		for($i=0;$i<$n;$i++) {
			...
			if(isset($_POST[$name]) && $_POST[$name]!='') {
				$query2 = "INSERT INTO `dhub_poll_responses` (`qid`,`response`) VALUES ('".$qid."','".$_POST[$name]."')";
				$cndb->dbQuery($query2);
				if($cndb->errorNo()>0) $err .= $cndb->errorNo().": ".$cndb->error()."<br />";
			}
		}
	...
	if($formname == 'poll_edit'){	
		$query = "UPDATE `dhub_poll_questions` SET `subject`='".$subject."',`question`='".$question."',`show`=".$cur.", `published`=".$pub." WHERE `qid`=".$_POST['qid'];
		//echo $query; exit;
		$cndb->dbQuery($query);
		if($cndb->errorNo()>0) $err .= $cndb->errorNo().": ".$cndb->error()."<br />";
		...
		for($i=0;$i<$n;$i++) {
			$name = "ans$i";
			if(isset($_POST[$name]) && $_POST[$name]!='') {
				$query2 = "SELECT `rid` FROM `dhub_poll_responses` WHERE `rid`=".$i." AND `qid`=".$_POST['qid'];
				$result2 = $cndb->dbQuery($query2);
				if( $cndb->recordCount($result2)>0) {
					$query3 = "UPDATE `dhub_poll_responses` SET `response`='".$_POST[$name]."' WHERE `rid`=".$i;
					$cndb->dbQuery($query3);
					if($cndb->errorNo()>0) $err .= $cndb->errorNo().": ".$cndb->error()."<br />";
					
				} else {
					$query3 = "INSERT INTO `dhub_poll_responses` (`qid`,`response`) VALUES ('".$_POST['qid']."','".$_POST[$name]."')";
					$cndb->dbQuery($query3);
					if($cndb->errorNo()>0) $err .= $cndb->errorNo().": ".$cndb->error()."<br />";
				}
			}
...
if ($formname=="edit_photo") 
{
	...
	$qry_delete = "";
		...
		//{
			...
			$sqpost[] = "update `dhub_dt_gallery_photos` set "
				  . "`title` = ".quote_smart($caption).", "
				  . "`description` = ".quote_smart($desc).", "
				  . "`id_gallery_cat` = '".$id_gallery_cat."', "
				  . "`published` = '".$published."' "
            ...
            if(strlen($_FILES['s_card']['name'][$doc_key]) > 4) 
			{
				...
				if(array_key_exists('id_parent', $post) and  is_array($post['id_parent']))
				{
					...
					if(count($menu_parent)>0)
					{
						...
						{  
						  if($menu_parent[$i] <> '') {
							$seq_update_menu[]  = " insert IGNORE into `dhub_dt_downloads_to_menus` ( `id_portal`, `id_menu`, `id_download` ) values "
				...
				if(array_key_exists('id_content', $post) and  is_array($post['id_content']))
				{
					...
					if(count($content_parent)>0)
					{
						...
						{  
							if($content_parent[$i] <> '') {
							$seq_update_content[]  = " insert IGNORE into `dhub_dt_downloads_to_contents` (`id_portal`, `id_content`, `id_download` ) values "
...
****************************************************************/
	...
	$seq_update_parent[]  = " insert IGNORE into `dhub_dt_downloads_parent` ( `id_download`, `id_menu` ) values "
	...
	if(array_key_exists('id_content', $post) and  is_array($post['id_content']))
	{
		...
		$seq_update_parent[]  = " insert IGNORE into `dhub_dt_downloads_parent` ( `id_download`, `id_content` ) values "
." ('".$id_download."', '".$content_parent[$i]."');  ";	
	...
	{
		$sq_par_clean = " delete from `dhub_dt_downloads_parent` where `id_download` = '".$id_download."'; ";
		$type->inserter_plain($sq_par_clean);
...
********************************************************************/
	...
	****************************************************************/
	...
	if(trim($_FILES['upload_icon']['name']) <> '') 
	{
		...
		if($the_image['result'] == 1)
		{
			/*$sq_upload = "insert into `dhub_dt_gallery_photos`  "
			."( `title`, `filename`,  `id_gallery_cat`, `date_posted`)   values "
			."(".quote_smart($post_title).", '".$the_image['name']."', 5, CURRENT_TIMESTAMP())";
			$type->inserter_plain($sq_upload);
			$id_photo = $type->qLastInsert;
			
			$sq_upload_parent = " REPLACE into `dhub_dt_gallery_photos_parent` " 
...
****************************************************************/
	...
	if($post['file_type'] == 'v')
	{
		...
		//`id_content` ,".quote_smart($id_content).", 
			$seq_post_a = "insert into `dhub_dt_gallery_photos`  "
	...
	if($post['file_type'] == 'u')
	{
		...
		{	
			...
			$sq_pic_rec = " select id from `dhub_dt_gallery_photos` where `filename` = ".quote_smart($post['photo_url_name'])."   limit 0, 1  ";
			...
			{
				$sqpost="insert into `dhub_dt_gallery_photos`  "
	...
	if($id_gall_item <> '')
	{
		...
		$seq_pic_parent[]  = " insert IGNORE into `dhub_dt_gallery_photos_parent` ( `id_photo`,`id_content`,`rec_stamp` ) values (".$id_gall_item.", ".quote_smart($id_content).", CURRENT_TIMESTAMP() );  ";			
...
if ($formname=="gal_category_new" or $formname=="gal_category_edit") 
{
	...
	{
		$sqpost="insert into `dhub_dt_gallery_albums`  (`id_type`,`id_parent1`, `id_parent2`, `title`, `description`, `published`) values
		 ('".$id_type."', '".$id_parent1."', '".$id_parent2."', '".$title."', '".$description."', '".$published."')";
	}
	elseif ($formname=="gal_category_edit") 
	{
		$sqpost="update `dhub_dt_gallery_albums` set  `id_type` ='".$id_type."', `id_parent1` ='".$id_parent1."', `id_parent2` ='".$id_parent2."', `title` ='".$title."', `description` ='".$description."', `published` ='".$published."'  where `id`='".$id."'"; 
	}
	//echo $sqpost; exit;
...
if ($formname=="admins_edit" or $formname=="admins_new") 
{
	...
	{
		$sqpost="insert into `dhub_admin_accounts`  (`admin_uname`, `admin_email`, `admin_fname`, `admin_pword`, `admintype_id`, `published`) values
	...
	elseif ($formname=="admins_edit") 
	{
		...
		$sqpost="update `dhub_admin_accounts` set  `admintype_id` ='".$admintype_id."', `admin_uname` ='".$admin_uname."', `admin_email` ='".$admin_email."', `admin_fname` ='".$admin_fname."',  `published` ='".$published."' $change_pass  where `admin_id`='".$id."'"; 
	}
	//echo $sqpost; exit;
...
if ($formname=="admin_log") 
{
	...
	/*$sqpost= "SELECT dhub_admin_accounts.admin_uname, dhub_admin_accounts.admin_pword, dhub_admin_accounts.admin_fname,  "; 
	$sqpost.= "dhub_admin_accounts.admintype_id, dhub_admin_accounts.admin_id, dhub_admin_types.title, "
		." dhub_admin_accounts.admin_email  FROM dhub_admin_accounts  "; 
	$sqpost.= "INNER JOIN dhub_admin_types ON dhub_admin_types.admintype_id=dhub_admin_accounts.admintype_id  "; 
	$sqpost.= " where  dhub_admin_accounts.admin_uname=".quote_smart($admin_uname)." and dhub_admin_accounts.admin_pword='".$md5_pword."' and dhub_admin_accounts.published = 1";*/
	
	$sqpost= "SELECT `dhub_admin_accounts`.*, `dhub_admin_types`.`title` FROM `dhub_admin_accounts` INNER JOIN `dhub_admin_types` ON (`dhub_admin_accounts`.`admintype_id` = `dhub_admin_types`.`admintype_id`) WHERE (`dhub_admin_accounts`.`admin_email` =".quote_smart($admin_uname)." AND `dhub_admin_accounts`.`admin_pword` = '".$md5_pword."' AND `dhub_admin_accounts`.`published` =1);"; 
	//echo $sqpost; exit;
	...
	if ($cnCnt==1) {//$username= substr($rscheck['email'], 0, stripos($rscheck['email'],"@"));
		...
		/*$sq_datelog	= "update `dhub_admin_accounts`  set `date_lastlog` = '".time()."' where `admin_id` = ".quote_smart($admin_id)." ";
		$cndb->dbQuery($sq_datelog);*/
		
		$_SESSION['sess_dhub_admin'] = $adm_sess;
...
if ($signout=="on") 
	{
		unset($_SESSION['sess_dhub_admin']);
