...
class data_arrays extends master
{
	...
	***********************************************************/
	...
	function buildMenu_Arr()
	{
		...
		$menus_modstamp = @$_SESSION['sess_dhub_menus']['_modstamp'];
		
		$rs_check = $this->dbQuery("SELECT `cache_id`, `cache_date`, `cache_data` FROM `dhub_cache_vars` where `cache_id` = 'menuChest'"); 
		...
		{ 
			...
			if($cache_date > $menus_modstamp)
			{
				...
				$_SESSION['sess_dhub_menus'] = 	$cache_data;
			}
			else
			{
				$cache_data = $_SESSION['sess_dhub_menus'];
			}
		}
	...
	function buildMenu_Main ($com_active, $id_menu_type = 1, $id_parent = 0 , $nav_type="mainNav")
	{   
		$out 		  	   = "";
		$nav_class_main    = "";
		$nav_class_sub     = "";
		...
		if(is_array($menu))
		{
			foreach($menu as $key => $val )	//=> $ml
			{
				...
				{ $ml_subs = count(master::$menusChild[$val]); $ml_has_subs = ' sf-with-ul'; }
	...
	function siteMenu_portal($id_portal = 1)
	{
		...
		." `".$pdb_prefix."dt_menu`.`id_type_menu` ASC, `dhub_dt_menu_parent`.`id_parent` ASC,  `".$pdb_prefix."dt_menu`.`title` ASC "; 
		...
		if($rs_mainlinks_count>=1)
		{
			...
			while($cn_mainlinks=$this->fetchRow($rs_mainlinks))
			{
				...
				$sq_chld = "SELECT `dhub_dt_menu_parent`.`id_menu` FROM `dhub_dt_menu_parent` inner join `".$pdb_prefix."dt_menu` on `dhub_dt_menu_parent`.`id_menu` = `".$pdb_prefix."dt_menu`.`id` where `dhub_dt_menu_parent`.`id_parent`= '".$id_link."' and `".$pdb_prefix."dt_menu`.`published` =1 ";
				//`dhub_dt_menu_parent`.`id_portal` = `".$pdb_prefix."dt_menu`.`id_portal` and 
	...
	function siteMenuSub_portal($id_parent = NULL)
	{
		...
		if($id_parent)
		{
			$crit = " AND (`dhub_dt_menu_parent`.`id_parent` = '".$id_parent."')  "; 
			//AND (`".$pdb_prefix."dt_menu`.`id_access` =1)
			...
			." `dhub_dt_menu_parent`.`id_parent` ASC, `".$pdb_prefix."dt_menu`.`title` ASC ";
			//echo $sq_sublinks;
			...
			if($rs_sublinks_count >=1)
			{
				...
				while($cn_sublinks=$this->fetchRow($rs_sublinks))
				{
					...
					$sq_chld = "SELECT `dhub_dt_menu_parent`.`id_menu` FROM `dhub_dt_menu_parent` inner join `".$pdb_prefix."dt_menu` on `dhub_dt_menu_parent`.`id_menu` = `".$pdb_prefix."dt_menu`.`id`  where `dhub_dt_menu_parent`.`id_parent`= '".$id_link."' and `".$pdb_prefix."dt_menu`.`published` =1 ";
					// and `dhub_dt_menu_parent`.`id_portal` = `".$pdb_prefix."dt_menu`.`id_portal`
					$rs_chld = $this->dbQuery($sq_chld);
	...
	***********************************************************/
	...
	function buildContent_Arr()
	{
		...
		$content_modstamp = @$_SESSION['sess_dhub_content']['_modstamp'];
		
		$rs_check = $this->dbQuery("SELECT `cache_id`, `cache_date`, `cache_data` FROM `dhub_cache_vars` where `cache_id` = 'contentChest'"); 
		...
		{ 
			...
			{
				$cache_data  = unserialize($cn_check['cache_data']);	
				$_SESSION['sess_dhub_content'] = 	$cache_data;
			}
			else
			{
				$cache_data = $_SESSION['sess_dhub_content'];
			}
	...
	function siteContentDates($cont_id)
	{
		...
		$sq_eventlinks = "SELECT DATE_FORMAT(`date`, '%Y%m%d') AS `ev_date`, DATE_FORMAT(`date`,'%l:%i %p') AS `ev_time_start`,  DATE_FORMAT(`end_date`, '%l:%i %p') AS `ev_time_end`, UNIX_TIMESTAMP(DATE_FORMAT(`date`, '%Y%m%e')) AS `ev_date_unix` FROM dhub_dt_content_dates WHERE (`id_content` =".quote_smart($cont_id).") ORDER BY `ev_date_unix` ;";
	...
	***********************************************************/
	...
	function siteGallery()
	{
		...
		$cat_modstamp = @$_SESSION['sess_dhub_gallery']['_modstamp'];		
		$rs_check = $this->dbQuery("SELECT `cache_id`, `cache_date`, `cache_data` FROM `dhub_cache_vars` where `cache_id` = 'galleryChest'"); 
		...
		{ 
			...
			{ 
				...
				$_SESSION['sess_dhub_gallery'] = 	$dataGallery;
			}
			else
			{  
				$dataGallery = $_SESSION['sess_dhub_gallery'];
			}
		}
	...
	***********************************************************/
	...
	function siteDocuments()
	{
		...
		$cat_modstamp = @$_SESSION['sess_dhub_resources']['_modstamp'];		
		$rs_check = $this->dbQuery("SELECT `cache_id`, `cache_date`, `cache_data` FROM `dhub_cache_vars` where `cache_id` = 'resourceChest'"); 
		...
		{ 
			...
			{ 
				...
				$_SESSION['sess_dhub_resources'] = 	$dataResources;
			}
			else
			{  
				$dataResources = $_SESSION['sess_dhub_resources'];
			}
		}
    ...
    , `dhub_dt_downloads`.`date_posted`
    , `dhub_dt_downloads`.`title`
    , `dhub_dt_downloads`.`description`
    , `dhub_dt_downloads`.`link` as `filename`
    , `dhub_dt_downloads`.`dtype` as `filetype`
    , `dhub_dt_downloads`.`dsize` as `filesize`
    , `dhub_dt_downloads`.`hlight`
    , `dhub_dt_downloads`.`hits`
	, `dhub_dt_downloads`.`link_seo`
FROM
    `dhub_dt_downloads`
    INNER JOIN `dhub_dt_downloads_parent` 
        ON (`dhub_dt_downloads`.`id` = `dhub_dt_downloads_parent`.`resource_id`)
...
ORDER BY `dhub_dt_downloads`.`date_updated` DESC;";
	...
	***********************************************************/
	function siteEventsFuture()
	{
		...
		$sq_eventlinks = " SELECT `dhub_dt_content_dates`.`id_content`,    DATE_FORMAT(`dhub_dt_content_dates`.`date`,'%b %e %Y') AS `date`, `dhub_dt_content`.`title`,  `dhub_dt_content`.`arr_extras` AS `location` FROM `dhub_dt_content_dates` INNER JOIN `dhub_dt_content`  ON (`dhub_dt_content_dates`.`id_content` = `dhub_dt_content`.`id`) WHERE (`dhub_dt_content`.`published` =1) GROUP BY `dhub_dt_content`.`title`, `dhub_dt_content_dates`.`id_content` ORDER BY `dhub_dt_content_dates`.`date` DESC ";
		// `dhub_dt_content_dates`.`date` >=CURRENT_DATE() and 
		$rs_eventlinks = $this->dbQuery($sq_eventlinks);
		$rs_eventlinks_count = $this->recordCount($rs_eventlinks);
		$truncFilter 	= "<img>";
