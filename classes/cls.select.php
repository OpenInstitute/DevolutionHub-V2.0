...
class drop_downs extends master
{
	...
	function checks($tbl, $col1, $col2, $title, $crit){ 		
		...
		$sq_more = "";
		if($tbl == 'dhub_reg_cats') { $sq_more = " and system_cat = '0' "; }
...
********************************************************************/		
	...
	{ // dropper_select
		...
		if($tblFilta == 'dhub_app_') { $sqPublished = ""; }
	...
	{ 
		...
		$result = $this->dbQuery("SELECT `choice_item` FROM `dhub_conf_choices` $sq_crit ;");
	...
	function drop_sel($tb_key, $crit='', $label='') {
		...
		switch($tb_key){
			case "jobgroup":   $attr = array("dhub_conf_jobgroup", "id_jobgroup", "jobgroup"); break;			
			case "bank_code":  $attr = array("dhub_conf_banks", "bank_code", "title");  break;			
			case "ward": 	   $attr = array("dhub_conf_wards", "id_ward", "ward");  break;
			case "member": 	 $attr = array("dhub_member", "id_member", "m_name");  break;
			case "committee":  $attr = array("dhub_committee", "id_committee", "title");  break;
			case "region":     $attr = array("dhub_conf_regions", "id_region", "region");  break;
		}		
	...
	{ 
		...
		$result = $this->dbQuery("SELECT `id_type`, `type_title` FROM `dhub_conf_types_detail` $sq_crit ;");
...
/* --------------------------------------------------------------------------------------------- */	
	...
	{ 
		...
		/*$sq = "SELECT `content_type` FROM `dhub_dt_downloads` WHERE `published` = 1 and `content_type` <>'' GROUP by `content_type` ORDER by `content_type` ";*/
		
		$sq = "SELECT `download_type` FROM `dhub_dt_downloads_type`  WHERE `download_type` <> '';";
	...
	{ 
		...
		$sq = "SELECT `id`, `title` FROM `dhub_dd_sections` $sq_crit ORDER BY `seq` ASC, `title` ASC ;";
...
/* --------------------------------------------------------------------------------------------- */	
	...
	function populateProjectLinks($link_type, $link_id, $sector_id, $project_id = '') 
	{
		...
		if($link_type <> '' and $link_id <> '' and $sector_id <> '' )
		{
			...
			$sq_clean = " delete from `dhub_app_project_links` where `".trim($link_type)."` = ".quote_smart($link_id)."; ";
			$rs_clean = $this->dbQuery($sq_clean); 
		
			$sq_query = "INSERT INTO `dhub_app_project_links` (`sector_id`,`project_id`,`".trim($link_type)."` ) values (".quote_smart($sector_id).", ".quote_smart($project_id).", ".quote_smart($link_id)." );  ";
	...
	function getProjectLinks($link_type, $link_id) 
	{
		...
		{
			$sq_query = "SELECT `sector_id` , `project_id`, `".trim($link_type)."` FROM `dhub_app_project_links` WHERE (`".trim($link_type)."` = ".quote_smart($link_id).")";
			$rs_query = $this->dbQuery($sq_query); 	
	...
	function getProjectParents($cat, $crit) 
	{
		...
		if($cat == 'sectors') { $tb_name = 'dhub_app_sector'; $tb_key  = 'sector_id'; $tb_col = 'title'; }
		if($cat == 'pillars') { $tb_name = 'dhub_app_pillar'; $tb_key  = 'pillar_id'; $tb_col = 'title'; }
		if($cat <> '')
		{
			//$this->connect() or trigger_error('SQL', E_USER_ERROR);
	...
	function getProjectList($sector_id)
	{
		...
		if($sector_id <> '')
		{
			$sq_data = "SELECT `project_id`, `pname` as `project_name` FROM `dhub_app_project` WHERE (`published`=1 and `sector_id` = ".quote_smart($sector_id)."); ";
		//echo $sq_data;
    ...
    , `dhub_app_location`.`name` AS `location`
FROM
    `dhub_app_project`
    ...
    LEFT JOIN `dhub_app_location` 
        ON (`dhub_app_project`.`location_id` = `dhub_app_location`.`location_id`)
WHERE (`dhub_app_project`.`project_id` = ".quote_smart($project_id)." AND `dhub_app_project`.`published` =1); ";
		//echo $sq_data;
			$rs_data = $this->dbQuery($sq_data);
			if($this->recordCount($rs_data)) 
			{
	...
	function getProjectComponents($project_id)
	{
		...
		{
			$sq_data = "SELECT
    `dhub_app_project_component`.`published`
    , `dhub_app_project_component`.*
    , `dhub_app_location`.`name` AS `location`
    , `dhub_app_location`.`lon` AS `longitude`
    , `dhub_app_location`.`lat` AS `latitude`
FROM
    `dhub_app_project_component`
    LEFT JOIN `dhub_app_location` 
        ON (`dhub_app_project_component`.`location_id` = `dhub_app_location`.`location_id`)
WHERE (`dhub_app_project_component`.`published` =1 AND `dhub_app_project_component`.`project_id` = ".quote_smart($project_id)."); ";
		//echo $sq_data;
			$rs_data = $this->dbQuery($sq_data);
			if($this->recordCount($rs_data)) 
			{
	...
	function getProjectGallery($project_id)
	{
		...
		if($project_id <> '')
		{
			//$sq_data = "SELECT `sector_id` , `project_id` , `id_gallery` FROM `dhub_app_project_links` WHERE (`project_id`  = ".quote_smart($project_id)." AND `id_gallery` <> 0); ";
			
			$sq_data = "SELECT `sector_id` , `project_id` , `id_content`, `id_gallery` FROM `dhub_app_project_links` WHERE (`project_id`  = ".quote_smart($project_id)."); ";
			$rs_data = $this->dbQuery($sq_data);
			if($this->recordCount($rs_data)) 
			{
				while($cn_data = $this->fetchRow($rs_data))
...
/* --------------------------------------------------------------------------------------------- */	
	...
	function populateContentParent($id_content, $array_parent) 
	{
		...
		if(is_array($array_parent) and $id_content <> '')
		{
			...
			$sq_clean = " delete from `dhub_dt_content_parent` where `id_content` = ".quote_smart($id_content)."; ";
			$rs_clean = $this->dbQuery($sq_clean); 
		
			foreach($array_parent as $kval) {  
				$sq_query = "insert IGNORE into `dhub_dt_content_parent` (`id_content`,`id_parent` ) values (".quote_smart($id_content).", ".quote_smart($kval)."); ";
				$rs_query = $this->dbQuery($sq_query); 
			} 
...
/* --------------------------------------------------------------------------------------------- */	
	...
	function populateKeywords($parent_type, $parent_id, $array_keys) 
	{
		...
		$sq_clean = " delete from `dhub_log_keywords` where `parent_type`=".quote_smart($parent_type)." and `parent_id`=".quote_smart($parent_id)." ";
		$rs_clean = $this->dbQuery($sq_clean); 
		
			foreach($array_keys as $kval) 
			{  
				$sq_query = " insert IGNORE into `dhub_log_keywords` (`keyword`, `parent_type`, `parent_id` ) values "
...
********************************************************************/		
	...
	function selectStatsCats($crit = '', $cat_parent = '', $cat_main = 1){ 
		...
		$sq_qry = "SELECT `id`, `title` FROM `dhub_stats_cats` WHERE `published` = 1 " . $sq_more;
...
********************************************************************/		
	...
	function selectDirCategory($crit = '', $cat_parent = ''){ //, $cat_dir = '', $cat_equip = ''
		...
		if($cat_parent == '_dir') { $sq_more = " and `dhub_reg_directory_category`.`cat_directory` = '1' "; }
		if($cat_parent == '_eqp') { $sq_more = " and `dhub_reg_directory_category`.`cat_equipment` = '1' "; }
		if($cat_parent == '_crp') { $sq_more = " and `dhub_reg_directory_category`.`cat_crop` = '1' "; }
		
		//$sq_qry = "SELECT `id`, `title`, `id_menu`, `cat_directory`, `cat_equipment`, `description`, `published`, `seq` FROM `dhub_reg_directory_category`  WHERE `published` = 1 " . $sq_more;
		
		$sq_qry = "SELECT `dhub_reg_directory_category`.`id`, `dhub_reg_directory_category`.`title`, `dhub_dt_menu`.`title` AS `menu` FROM `dhub_reg_directory_category` LEFT JOIN `dhub_dt_menu` ON (`dhub_reg_directory_category`.`id_menu` = `dhub_dt_menu`.`id`)  WHERE `dhub_reg_directory_category`.`published` = 1 " . $sq_more;
	...
	{ 
		...
		$sq = "SELECT `conf_data_id` , `conf_data_title` FROM `dhub_conf_types_data` WHERE (`published` =1 AND `conf_type_id` = ".quote_smart($cat).") ORDER BY `conf_data_title` ASC, `seq` ASC;";
	...
	function selectDirRegion($crit = ""){ 
		//$this->connect() or trigger_error('SQL', E_USER_ERROR);
		$result=$this->dbQuery("SELECT `ac_country` FROM `dhub_reg_directory` GROUP BY `ac_country`");
	...
	****************************************** */ 
	...
	function getAddUserAccount($account_email, $account_arr=array(), $mailing = 0) 
	{
		...
		{
			//`dhub_reg_users`
			$sq_check = "SELECT `account_id`, `email` FROM `dhub_reg_account` WHERE (`email` = ".quote_smart($account_email).")";
			...
			/* ----------------------------------------- */
				...
				$sqpost = "insert ignore into `dhub_reg_account` ($field_title `email`, `ipaddress`, `published`) values 
	...
	****************************************** */ 
	function getAddResourceType($type_name, $type_arr = array()) 
	{
		...
		if($type_name <> '')
		{
			...
			$sq_check 	= "SELECT `res_type_id` FROM `dhub_dt_downloads_type` WHERE `res_type_seo`=".q_si($type_seo).";";
			$rs_check 	= $this->dbQuery($sq_check); 
			...
			{	
				...
				$resource_id = 0;
				$sqpost = "insert into `dhub_dt_downloads_type` ($field_title `resource_id`) values 
	...
	****************************************** */ 
	function getAddOrganization($org_name, $org_arr = array()) 
	{
		...
		if($org_name <> '')
		{
			...
			$sq_check 	= "SELECT `organization_id` FROM `dhub_conf_organizations` WHERE `organization_seo`=".q_si($org_seo).";";
			$rs_check 	= $this->dbQuery($sq_check); 
			...
			{	
				...
				$is_partner = 0;
				$sqpost = "insert into `dhub_conf_organizations` ($field_title `is_partner`) values 
	...
	function getAddUserOrganization($ac_domain, $contact_id) {
		...
		if($ac_domain <> '')
		{
		$sq_check = "SELECT `organization_id` FROM `dhub_conf_organizations` WHERE `organization`=".q_si($ac_domain)." limit 1;";
		$rs_check = $this->dbQuery($sq_check);
			...
			{	
				$sqpost = "insert into `dhub_conf_organizations` (`organization`, `organization_seo`, `contact_id`) values 
	...
	****************************************** */ 
	...
	function getAddUserCat($account_cat) {
		...
		if($account_cat <> '')
		{
			...
			$sq_check = "SELECT `id_category` FROM `dhub_reg_cats` WHERE `title` = ".quote_smart($account_cat)."; ";
			$rs_check = $this->dbQuery($sq_check);
			...
			{	
				$sqpost = "insert into `dhub_reg_cats` (`title`, `title_url`) values 
	...
	****************************************** */ 
	...
	function addUserToCategory($cat_id, $account_id, $pref_arr = array()) {
		...
		{
			$sqpost = "replace into `dhub_reg_cats_links` ($pref_title `id_category`, `account_id`) values 
	...
	function selectUserCat($multi = "y", $crit = "") { 
		...
		$out = '';	
		$qry_links ="SELECT `id_category`, `title`, `published` FROM `dhub_reg_cats` WHERE  `published` =1 ORDER BY   `title` ASC ";
	...
	****************************************** */ 
	...
	function selectCountry($crit) {
		$country = '';
		//$this->connect() or trigger_error('SQL', E_USER_ERROR);
		$rs_data= $this->dbQuery("SELECT `id`, `country` FROM `dhub_reg_countries` WHERE `id`=".quote_smart($crit)." or `iso_code_2`=".quote_smart($crit)."  or `iso_code_1`=".quote_smart($crit)." "); //
	...
	****************************************** */ 
	...
	function selectDownloads($crit = "") { 
		...
		$sqList = "SELECT
    `dhub_dt_downloads`.`id`
    , `dhub_dt_downloads`.`title`
    , `dhub_dt_downloads`.`link`
    , `dhub_dt_downloads`.`published`
FROM
    `dhub_dt_downloads` WHERE `dhub_dt_downloads`.`published` = 1
ORDER BY `dhub_dt_downloads`.`title`;";
	...
	****************************************** */ 
	...
	function selectThematicAreas($crit) { 
		...
		$result=$this->dbQuery("SELECT `id`, `title` FROM `dhub_dt_menu` WHERE `published`= 1 and `id_section` = 7  order by `title`; ");
	...
	****************************************** */ 
	...
	function selectThemeTabs($crit){ 
		...
		$result=$this->dbQuery("SELECT `id`, `title` FROM `dhub_dt_menu` WHERE `published`= 1 and `id_type_menu` = 7  order by `seq`; ");
...
/*-----------------------------------------------------------------------------------*/	
	...
	{	
		...
		$rs_qry = $this->dbQuery("SELECT `ac_organization`, `ac_email`,  `ac_contact_name`, `ac_contact_email` FROM `dhub_reg_directory` WHERE `id` = ".quote_smart($supplier_id)."; ");
...
// oi_edit_log: Kev Edit - Added user account function from manifesto to devhub - 10th March 2017

	/* ****************************************
	 @Automation  - GET /ADD USER ACCOUNT
	****************************************** */ 
	
	function getAddSocialAccount($social_type, $social_id, $account_arr=array()) 
	{
		...
		{
			//`dhub_reg_users`
			$sq_check = "SELECT `account_id`, `social_id` FROM `dhub_reg_account` WHERE (`social_type` = ".quote_smart($social_type)." and `social_id` = ".quote_smart($social_id).")";
			$rs_check = $this->dbQuery($sq_check);
			...
			/* ----------------------------------------- */	
				...
				if(is_array($account_arr))
				{
					foreach($account_arr as $col=>$value)
					{	
						$field_title .= " `$col`, "; $field_value .= " ".quote_smart($value).", ";
					}
				}
			/* ----------------------------------------- */
			
			
				$sqpost = "insert ignore into `mfsto_dt_accounts` ($field_title `ip_address`, `published`) values 
				($field_value  ".quote_smart($_SERVER['REMOTE_ADDR']).", '1' ) ";				
				$result = $this->dbQuery($sqpost);
				$auth_id = $this->insertId();				
			}
