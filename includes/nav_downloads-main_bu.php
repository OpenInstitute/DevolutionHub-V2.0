		...
		$section_items = array_filter($jsonResourcesArray, function ($x) use ($terms, $dir_category){
			foreach($terms as $tcol => $term){
				...
				if($term <> '')
				{
					...
					if($terms['words'] <> '') { 
						if (stripos($x["cont_brief"], $term) || $x["cont_title"] == $term || stripos($x["cont_title"], $term) ||  stripos($x["cont_type"], $term)|| stripos($x["cont_author"], $term)) 
						{ $isThere  = 1;  }
					}
					
					
					if($terms['cat'] <> '' and $x["cont_type"]== $terms['cat']) { $isThere  = 1; }
					if($terms['year'] <> '' and $x["year_published"] == $terms['year']) { $isThere = 1; } //stripos($x["year_published"], $dir_year)
					if($terms['county'] <> '' and trim($x["county"]) == $terms['county']) { $isThere = 1;  }
					if($terms['organization'] <> '' and trim($x["cont_author"]) == $terms['organization']) { $isThere = 1;  }
