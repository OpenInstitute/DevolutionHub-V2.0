...
$u_organization_id = ($sess_mbr['u_organization_id'] <> '') ? $sess_mbr['u_organization_id'] : '';
	...
	$sq_crit = " WHERE `dhub_dt_content`.`published` = '1' AND `dhub_dt_content`.`approved` = '1'  AND `dhub_dt_content`.`id_owner` = ".q_si($us_id)."  ";
if($us_org_id <> ''){
	$sq_crit .= " OR `dhub_dt_content`.`published` = '1' AND `dhub_dt_content`.`approved` = '1' AND `dhub_dt_content`.`organization_id` = ".q_si($us_org_id)."  ";
}
