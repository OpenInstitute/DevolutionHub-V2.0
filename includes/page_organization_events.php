<?php
$contCom 		= $dispData->siteEventsList($fData['organization_id']);
$contArr 		=  $contCom; 
...
if(count($section_pages))
{
	...
	foreach ($contPages as $kk => $contKey) 						
	{
		...
		if(is_array($event_dates) and count($event_dates) > 0)
		{
			...
			$org_disp   = '<div><span class="postDate nocaps">'. ucwords($cont_location) . '' . $cont_organization .'</span></div>';
...
-------------------------------------------------------------------------------------------------------*/ 
	...
	$itemViews = '<div><span class="postDate">'.$date_label.''.$cont_date.'</span> </div>'; // | Viewed '.$cont_hits.' times
	$itemBreak = 'news-bits';
